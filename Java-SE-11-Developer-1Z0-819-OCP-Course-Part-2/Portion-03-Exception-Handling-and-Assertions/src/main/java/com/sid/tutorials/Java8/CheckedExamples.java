...
public class CheckedExamples {
	...
	public static void main(String[] args) throws CustomThrowable, CustomException {
	...
	private void throwThrowable(int i) throws CustomThrowable {
		try {
			...
			if (i == 0)
				throw new CustomThrowable("Caught and Re-Thrown", e);

			// Note that exception is ignored if i <> 0
		}
		// Calls constructor on CustomThrowable that takes message
		if (i == 1)
			throw new CustomThrowable("Created and Thrown");
	...
	private void testExceptionNotHandled(int i) throws CustomException {
		try {
			...
			if (i == 0)
				throw new CustomException("Custom Exception", e, false);
			else
