...
public class MoviesClient {
	...
	public List<Movie> retrieveMovieList_CF_allOf(List<Long> movieInfoIds) {
		List<CompletableFuture<Movie>> movieFutures = movieInfoIds.stream().parallel().map(this::retrieveMovie_CF)
				.collect(Collectors.toList());
		CompletableFuture<Void> cfAllOf = CompletableFuture
				.allOf(movieFutures.toArray(new CompletableFuture[movieFutures.size()]));
		return cfAllOf.thenApply(v -> movieFutures.stream().map(CompletableFuture::join).collect(Collectors.toList()))
				.join();
	}
	...
	public List<Movie> retrieveMovieList_CF_anyOf(List<Long> movieInfoIds) {
		List<CompletableFuture<Movie>> movieFutures = movieInfoIds.stream().parallel().map(this::retrieveMovie_CF)
				.collect(Collectors.toList());
		CompletableFuture<Void> cfanyOf = CompletableFuture
				.allOf(movieFutures.toArray(new CompletableFuture[movieFutures.size()]));
		return cfanyOf.thenApply(v -> movieFutures.stream().map(CompletableFuture::join).collect(Collectors.toList()))
				.join();
	}
}
