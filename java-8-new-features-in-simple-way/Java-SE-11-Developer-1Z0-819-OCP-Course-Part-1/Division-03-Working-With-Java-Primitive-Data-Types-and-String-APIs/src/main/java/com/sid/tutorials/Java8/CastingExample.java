...
public class CastingExample {
	public static void main(String[] args) {
		...
		for (int i = minValue; i <= maxValue; i++) {
			...
			// We know because of the way we've written the code that i will never be
			// outside
			...
			if (i == minValue)
				System.out.println("Minimum Short value = " + s2);
			if (i == maxValue)
				System.out.println("Maximum Short value = " + s2);
	...
	private static void callAMethodWithParameter(int i, short s) {
		// Do something here with your short that you would not do with an int or long
		// for example

		// Really only want this to print once, though it's called in a loop...
		if (s == Short.MAX_VALUE)
			System.out.println("Iteration " + i + " had the max value, my type is short");
	}
